...
export async function proxy(request: NextRequest) {
  ...
  const bypassAuthForE2E =
    ...
    process.env.E2E_BYPASS_GUARD === 'allow-non-production' &&
  ...
  // E2E mode can bypass page-level auth gating while keeping API auth checks in routes.
  if (bypassAuthForE2E && !pathname.startsWith('/api/')) {
    return NextResponse.next();
  }
